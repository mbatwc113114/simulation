...
current_path = os.path.join(os.getcwd(), "Simulations")  # Corrected path join
if not os.path.exists(current_path):
    print(f"Error: Directory '{current_path}' not found!")
    exit(1)

folders = [f for f in os.listdir(current_path) if os.path.isdir(os.path.join(current_path, f)) and f not in excluded_folders]
...
def open_child(folder):
    if folder in child_script_paths:
        ...
        print(f"Loading script: {script_path}")  # Debugging
        ...
        try:
            ...
            if hasattr(module, "open_window"):
                ...
                print(f"Warning: {script_path} has no 'open_window(root)' function!")
        except Exception as e:
            print(f"Error loading {script_path}: {e}")

# Scrollable frame for buttons
frame = ctk.CTkScrollableFrame(root, width=780, height=250)
